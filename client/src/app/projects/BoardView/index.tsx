...
import useApiStore, { Task as TaskType} from "@/app/state/api";
...
const BoardView = ({ setIsModalNewTaskOpen, id }: BoardProps) => {
  const { tasks, isLoading, error, fetchTasks, updateTaskStatus } = useApiStore(
    useShallow((state) => ({ tasks: state.tasks, isLoading: state.isLoading, error: state.error, fetchTasks: state.fetchTasks, updateTaskStatus: state.updateTaskStatus}))
  )
...
useEffect(() => {
  ...
  if (error) return <div>Ocorreu algum erro ao carregar as tarefas...</div>;
...
const Task = ({ task }: TaskProps) => {
  console.log("Task Data:", task); // Log dos dados da tarefa
